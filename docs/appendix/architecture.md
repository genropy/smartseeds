...
Technical design and architecture of SmartSeeds.

## Overview

SmartSeeds is designed with simplicity and zero dependencies in mind:
...
**Location**: [src/smartseeds/decorators.py](../../src/smartseeds/decorators.py)
...
**Location**: [src/smartseeds/dict_utils.py](../../src/smartseeds/dict_utils.py)
...
SmartSeeds uses **only Python standard library**:
...
SmartSeeds uses a type-preserving decorator pattern:
...
└── test_decorators.py
    ...
    ├── TestExtractKwargsOptions    # Advanced options
    └── TestExtractKwargsEdgeCases  # Edge cases
```

**Coverage**: 96% (missing only unreachable defensive code)
