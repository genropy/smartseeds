...
```python
from smartseeds.ascii_table import render_ascii_table
...
```python
from smartseeds.ascii_table import render_markdown_table
...
- `coldef`: Column definition dict with `type` and optional `format`
...
**Parameters:**
- `fmt`: Format string with tokens like `yyyy`, `mm`, `dd`, etc.
...
**Parameters:**
- `value`: Value to parse (string, bool, int)

**Returns:** True, False, or original value if not parseable

## See Also

- [SmartOptions](smartoptions.md) - Option merging and filtering
